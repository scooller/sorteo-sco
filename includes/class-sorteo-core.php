...
class Sorteo_SCO_Core {
	...
	public function check_and_execute_auto_draw($order_id, $new_status) {
		...
		if ($min_ganancia <= 0) {
			error_log('Sorteo SCO: Ejecutando sorteo inmediato (mínimo ganancia = 0)');
	...
	private function execute_automatic_draw_for_order($order) {
		...
		error_log('Sorteo SCO: Ganador: ' . $winner_name . ' (' . $winner_email . ')');
		...
		error_log('Sorteo SCO: Guardado en historial. Total sorteos: ' . count($draws_history));
		error_log('Sorteo SCO: Draw data: ' . print_r($draw_data, true));
		
		// Actualizar métricas (ganancia total, costos, etc.)
		$this->update_metrics();
		
		error_log('Sorteo SCO: Métricas actualizadas');
		...
		error_log('Sorteo SCO: Meta data guardada en pedido #' . $order_id);
		...
		if (function_exists('WC') && WC()->session) {
			try {
				...
				error_log('Sorteo SCO: Sesión WC guardada');
			} catch (Exception $e) {
		...
		error_log('Sorteo SCO: Cookie configurada para order #' . $order_id);
		
		// Enviar email al ganador usando el order_id
		error_log('Sorteo SCO: Intentando enviar email...');
		error_log('Sorteo SCO: Order ID: ' . $order_id . ' | Prize: ' . $prize_name);
		error_log('Sorteo SCO: Email destino: ' . $winner_email);
		
		if (class_exists('Sorteo_SCO_Email')) {
			error_log('Sorteo SCO: Clase Sorteo_SCO_Email encontrada, llamando send_winner_email()');
			$email_sent = Sorteo_SCO_Email::send_winner_email($order_id, $prize_name);
			error_log('Sorteo SCO: Resultado envío email: ' . ($email_sent ? 'SUCCESS' : 'FAILED'));
			if (!$email_sent) {
		...
		error_log('Sorteo SCO: Enviando notificación a administradores...');
		$this->send_admin_notification_auto_draw($draw_data);
		
		error_log('Sorteo SCO: === SORTEO COMPLETADO ===');
	}
		...
		if (!empty($user_email)) {
			...
			foreach ($recent_orders as $order) {
				...
				if ($is_winner === '1' && $shown !== '1') {
					...
					update_post_meta($order->get_id(), '_sorteo_winner_shown', '1');
					error_log('Sorteo SCO: Mensaje mostrado para pedido #' . $order->get_id() . ' via post_meta');
					return true;
				}
			}
		}
