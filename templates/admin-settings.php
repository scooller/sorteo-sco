			...
			<table class="form-table">
				...
				<tr>
					...
					<td>
						...
						?>
						<textarea name="sorteo_sco_email_content" rows="8" style="width: 100%; max-width: 700px;"><?php echo esc_textarea($email_content); ?></textarea>
						<p class="description">
		...
		<div id="content-premios" class="tab-content">
			...
			if (!empty($draws_history)) {
				...
				foreach ($draws_history as $draw) {
					...
					// Traducir tipo
					$type_label = $type === 'automatic_immediate' ? __('Automático Inmediato', 'sorteo-sco') : 
								  ($type === 'automatic_threshold' ? __('Automático por Umbral', 'sorteo-sco') : 
		...
		<div id="content-logs" class="tab-content">
			...
			if (file_exists($debug_log_path)) {
				...
				if ($log_lines !== false) {
					...
					foreach ($log_lines as $line) {
						if (stripos($line, 'Sorteo SCO') !== false && 
